---
title: "Predicting Airbnb Prices From Bristol Airbnb Listings"
author: "Enric Fernández Sala"
date: "2025-03-26"
output:
...
This dataset includes various attributes that describe Airbnb listings from Bristol, such as price, location, number of rooms, and user reviews.
...
After pre-selecting the raw data we will be using, we need to clean it by addressing missing values. Additionally, some column's NAs were not formated correctly in order for R to detect them, so we must correct it.
...
We can observe below which variables have missing data.
...
Remove observations where the price is missing, since these will not allow us to either train or test our models.
...
By checking the distributions of some of the numeric variables with a high proportion of NAs we can see that they are skewed, thus using the median to impute NAs is more appropriate as it is not affected by extreme values.
...
We can check for a possible correlation variables missing values to see if we could impute the values of one for the other, but since it is weak, such imputation cannot be made. The same logic cannot be applied to review scores because they are missing in the same observations.
...
Therefore, to maintain the integrity of the dataset and facilitate the manipulation of a large number of observations, it is advisable to impute the median for NAs.
...
Tt is really skewed to the right, indicating that we have really high outliers and, although they don't represent a significant proportion of prices, they can compromise our predictions due to their extreme values. We can remove the 5% of higher prices to avoid this.
...
We can check for outliers in other features looking at boxplots.
...
Feature engineering allows us to derive more useful variables from the ones that we already have by giving them a different format or transforming them. For example, transforming binary variables to numerical format, getting host experience from the date column or dividing bathroom text variable into a numerical and binary (for shared/private).
...
Categorical variables like property type and room type have no inherent order, while response time is ordinal. We apply ordinal encoding for response time and one-hot encoding for the others. To reduce the high cardinality of property type, we apply a cutoff, simplifying the model.
...
The amenities column is rich with binary data (e.g., presence of WiFi, kitchen, etc.). We encode each amenity as a binary variable and remove any features that are present in fewer than 10% of listings to avoid overfitting and improve model generalization.
...
An optimal number of clusters could be 5 due to a lower marginal gain further on.
...
We can erase the columns we used to create better variables and check the final structure of the dataframe.
...
Standardizing the features ensures that each variable contributes equally to the model and avoids dominance of high-magnitude variables.
...
Lasso regression is a good model for predicting using this dataframe because it performs feature selection (+100 variables), manages multicolinearity (many features are correlated) and prevents overfitting with regularization, generalizing better to unseen data.

The first steps we can make is remove some categories and coordinates to avoid unnecessary colinearity, and impose an interaction to only use reviews when there has been one.
...
Next, we split the dataframe into training and testing sets and use 10 fold cross validation to obtain the optimal penalization parameter value.
...
Now we can train the model with the optimal lambda to make predictions and evaluate their performance on the training and testing sets.
...
This table contains the coefficients of the regression.
...
To understand better model performance we can plot the distribution of errors and predicted vs actual prices.
...
We can see how residuals are greater as price increases, which compromises the performance of the model. We can try a logarithmic transformation on prices to see if it improves.
...
The performance in the test set is approximately the same than without logarithm.
...
Let us impose interactions in reviews. GB does not need to remove variables with multicolinearity.
...
Cross-validation allows model fit and hyperparameter optimization.
...
The model overfits the data because the RMSE is substantialy lower in the training set.

Regarding the testing set, performance is very similar to LASSO regression, suggesting that there has been no improvement.
...
The pattern is the same as with LASSO, errors grow as price gets higher.

Finally, we can see in the feature importance's graph that "accommodates" and "bedrooms" helped the algorithm with prediction, repeatedly dividing the data in more homogeneous groups.
...
Both Lasso Regression and Gradient Boosting performed similarly, struggling to predict high-priced listings due to outliers and non-linearity. Lasso helped with feature selection but had high RMSE values, especially for higher prices. Gradient Boosting overfitted the training data, offering no significant improvement. Feature engineering, including outlier treatment and categorical encoding, improved the dataset’s structure, but more advanced models or transformations might be needed to better predict high-priced listings.
